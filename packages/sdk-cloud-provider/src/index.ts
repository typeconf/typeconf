...
export interface TypeconfCloudClient {
    ...
    readConfig<T>(projectNameOrId: string, configName: string): Promise<{ config?: T, err?: Error }>;
}
...
class TypeconfCloudClientImpl implements TypeconfCloudClient {
    ...
    public async readConfig<T>(projectNameOrId: string, configName: string): Promise<{ config?: T, err?: Error }> {
        try {
