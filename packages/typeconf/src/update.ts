...
import {TypeconfConfig} from '@typeconf/typeconf-sdk'
...
export async function updateConfig(): Promise<void> {
    ...
    const conf = await jiti.import(filepath, { default: true }) as TypeconfConfig;
...
function updateConfigFromDirectory(baseDir: string, configDir: string): void {
    fs.copyFileSync(path.join(configDir, "types", "all.ts"), path.join(baseDir, "typeconf-gen", "all.ts"));
}
