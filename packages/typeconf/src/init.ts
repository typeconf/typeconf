...
const TEMPLATES = new Map<string, any>([
    ["package.json", (params: PackageJsonParams) => {
        return `{
...
if (values != null && 'default' in values) {
  ...
  projectName: string;
    ...
    ["values.config.ts", (params: ValuesParams) => {
        return `import { ProjectConfig } from "./types/all.js";
...
export default async function initProject(projectPath: string) {
    ...
    if (fs.existsSync(projectPath)) {
        ...
        if (files.length > 0) {
            const confirm_resp = await prompts({
                ...
                message: 'Current directory is not empty, are you sure you want to initialize a project here?',
            });
    ...
    console.log('Done! Now you can edit main.tsp and set your configuration schema.');
}
